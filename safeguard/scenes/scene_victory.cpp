...
void VictoryScene::Load() {
    ...
    //LOGO TITLE
    {
        ...
        auto t = logo->addComponent<TextComponent>("VICTORY!");
        ...
        logo->setPosition(Vector2f(Engine::GetWindow().getSize().x / 2 + 10.0f, Engine::GetWindow().getSize().y / 2 - 300.0f));
    ...
    //RESTART
    {
        ...
        auto t1 = start->addComponent<TextComponent>("PLAY AGAIN");
        t1->getText()->setOrigin(t1->getText()->getLocalBounds().width / 2 + 60.0f, t1->getText()->getLocalBounds().height / 2 + 60.0f);
...
void VictoryScene::Update(const double& dt) {
    ...
    //RESTART 
    if (Engine::GetActiveScene()->ents.find("VictoryRestart")[0]->get_components<ButtonComponent>()[0]->isHighlighted() && !_selectionSoundPlayedRestart) {
        _selectSound.play();
        _selectionSoundPlayedRestart = true;
        Engine::GetActiveScene()->ents.find("VictoryRestart")[0]->get_components<ShapeComponent>()[0]->getShape().setScale(sf::Vector2f(1.2, 1.2));

    }
    else if (!Engine::GetActiveScene()->ents.find("VictoryRestart")[0]->get_components<ButtonComponent>()[0]->isHighlighted() && _selectionSoundPlayedRestart) {
        _selectionSoundPlayedRestart = false;
        Engine::GetActiveScene()->ents.find("VictoryRestart")[0]->get_components<ShapeComponent>()[0]->getShape().setScale(sf::Vector2f(1, 1));
    }

    //MAIN MENU
    if (Engine::GetActiveScene()->ents.find("VictoryMainMenu")[0]->get_components<ButtonComponent>()[0]->isHighlighted() && !_selectionSoundPlayedExitMenu) {
        _selectSound.play();
        _selectionSoundPlayedExitMenu = true;
        Engine::GetActiveScene()->ents.find("VictoryMainMenu")[0]->get_components<ShapeComponent>()[0]->getShape().setScale(sf::Vector2f(1.2, 1.2));

    }
    else if (!Engine::GetActiveScene()->ents.find("VictoryMainMenu")[0]->get_components<ButtonComponent>()[0]->isHighlighted() && _selectionSoundPlayedExitMenu) {
        _selectionSoundPlayedExitMenu = false;
        Engine::GetActiveScene()->ents.find("VictoryMainMenu")[0]->get_components<ShapeComponent>()[0]->getShape().setScale(sf::Vector2f(1, 1));
    }


    if (_clickMenuTimeout < 0.0f) {
        //RESTART
        if (Engine::GetActiveScene()->ents.find("VictoryRestart")[0]->get_components<ButtonComponent>()[0]->isSelected() && Mouse::isButtonPressed(Mouse::Left)) {
            _selectSound.play();
            _restartGame = true;
            _clickMenuTimeout = 0.5f;
        }
        //EXIT MENU
        if (Engine::GetActiveScene()->ents.find("VictoryMainMenu")[0]->get_components<ButtonComponent>()[0]->isSelected() && Mouse::isButtonPressed(Mouse::Left)) {
            _selectSound.play();
            _quitGameMenu = true;
            _clickMenuTimeout = 0.5f;
        }
